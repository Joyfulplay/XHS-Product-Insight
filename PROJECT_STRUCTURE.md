...
- `backend/app/crawlers/`：小红书本地采集模块，读取本机登录状态并采集公开笔记与评论。
- `backend/app/preprocess/`：数据预处理模块，包括文本清洗、图片处理、去重和标准化。
- `backend/app/schemas/`：Pydantic 数据契约，约束采集结果、清洗后笔记和分析结果结构。
- `backend/app/services/analysis_pipeline.py`：后端分析编排，组合本地分析与可选 OpenAI 洞察生成。
- `backend/app/services/localAnalysis.py`：无大模型配置时使用的本地清洗、统计、情感、风险和代表笔记分析。
- `backend/app/services/persistence_service.py`：结果持久化服务，在 `backend/.runtime/collections/` 保存和读取采集分析结果。
